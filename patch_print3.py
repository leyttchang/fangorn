...
with open('Y:/Fangorn/fangorn/components/health_component.gd', 'r', encoding='utf-8') as f:
    content = f.read()
content = content.replace('func take_damage(raw_damage: float) -> void:\\\n\\\tprint("TAKE DAMAGE CALLED! ", raw_damage)', 'func take_damage(raw_damage: float) -> void:')
content = content.replace('func take_damage(raw_damage: float) -> void:\n\tprint("TAKE DAMAGE CALLED! ", raw_damage)', 'func take_damage(raw_damage: float) -> void:')
...
with open('Y:/Fangorn/fangorn/components/knockback_componant.gd', 'r', encoding='utf-8') as f:
    content = f.read()
content = content.replace('func apply_knockback(push_direction: Vector3, raw_knockback_force: float) -> void:\\\n\\\tprint("APPLY KNOCKBACK CALLED! Force: ", raw_knockback_force)', 'func apply_knockback(push_direction: Vector3, raw_knockback_force: float) -> void:')
content = content.replace('func apply_knockback(push_direction: Vector3, raw_knockback_force: float) -> void:\n\tprint("APPLY KNOCKBACK CALLED! Force: ", raw_knockback_force)', 'func apply_knockback(push_direction: Vector3, raw_knockback_force: float) -> void:')
