    ...
    for block in blocks:
        if block.startswith("[sub_resource type=\"Resource\" id=\""):
            ...
            if stat_name:
                ...
                sign = "+" if v >= 0 else ""
                if mod_type == "1":
                    sub_res[res_id] = f"{sign}{value}% {stat_name} (Increase)"
                else:
                    sub_res[res_id] = f"{sign}{value} {stat_name} (Flat/More)"
    return sub_res

rows = []
headers = [
    "Filename", "Node ID", "Node Name", "Description", "Node Type",
...
for root, _, files in os.walk(directory):
    for f in files:
        if f.endswith(".tres"):
            ...
            # Robust multiline description matching
            # It matches from 'description = "' up to the next unescaped quote at the end of a line
            ...
            rows.append([
                f, node_id, node_name, description, node_type_str,
...
print(f"Exported clean CSV to {output_file}")
