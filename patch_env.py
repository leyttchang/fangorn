...
content = content.replace('''	# Si ce n'est pas attach un personnage, on vrifie si c'est un projectile gr par le serveur
	if p == null and multiplayer.is_server():
		is_active_for_network = true''', '''	# Si ce n'est pas attach un personnage (ex: Pige, Flche), TOUT LE MONDE a le droit d'activer la hitbox !
	# (Mais l'écho visuel des sorts sera désactivé manuellement par le SkillBarComponent)
	if p == null:
